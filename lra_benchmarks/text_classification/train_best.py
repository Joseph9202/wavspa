...
flags.DEFINE_integer(
    'override_nperseg', default=None, help='Alternative Nperseg')
...
def main(argv):
    ...
    if FLAGS.override_nperseg is not None:
        model_kwargs['nperseg'] = FLAGS.override_nperseg
    ...
    jax.profiler.save_device_memory_profile("{}/memory.prof".format(FLAGS.model_dir))
    ...
    with tf.io.gfile.GFile(os.path.join(FLAGS.model_dir, 'results.json'), 'w') as f:
        ...
        test_summary["step_per_sec"] = jnp.array(np.mean(step_lst))
        json.dump(jax.tree_map(lambda x: x.tolist(), test_summary), f)
